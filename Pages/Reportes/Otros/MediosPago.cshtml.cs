...
namespace ReservaCabanasSite.Pages.Reportes.Otros
{
    ...
    public class MediosPagoModel : PageModel
    {
        private readonly AppDbContext _context;
        private readonly IExportacionService _exportacionService;

        public MediosPagoModel(AppDbContext context, IExportacionService exportacionService)
        {
            _context = context;
        ...
        [BindProperty]
        public ReporteMediosPagoViewModel ReporteModel { get; set; } = new();
        ...
        private async Task CargarReporte()
        {
            ...
            // Agrupar por medio de pago y calcular estadísticas
            var ingresosPorMedioPago = reservas
                .GroupBy(r => string.IsNullOrEmpty(r.MetodoPago) ? "Sin especificar" : r.MetodoPago)
                .Select(g => new ReporteMedioPagoItem
                {
                    MedioPago = g.Key,
            ...
            // Calcular totales
            var totalReservas = ingresosPorMedioPago.Sum(r => r.CantidadReservas);
            var totalIngresos = ingresosPorMedioPago.Sum(r => r.TotalIngresos);

            // Calcular porcentajes
            foreach (var item in ingresosPorMedioPago)
            {
            ...
            ReporteModel.IngresosPorMedioPago = ingresosPorMedioPago;
        ...
        public async Task<IActionResult> OnGetExportarExcelAsync(DateTime? fechaDesde, DateTime? fechaHasta, int? cabanaId)
        {
            ...
            {
                TituloReporte = "Reporte de Ingresos por Medio de Pago",
            ...
            {
                new() { Nombre = "Medio de Pago", TipoDato = "string", Ancho = 30, Alineacion = "left" },
            ...
            // Agregar datos
            foreach (var item in ReporteModel.IngresosPorMedioPago)
            {
                var fila = new Dictionary<string, object>
                {
                    ["medio_pago"] = item.MedioPago,
                    ["cantidad_reservas"] = item.CantidadReservas,
                    ["total_ingresos"] = item.TotalIngresos,
                    ["promedio_ingreso"] = item.PromedioIngreso,
                    ["porcentaje_ingresos"] = item.PorcentajeIngresos / 100
                };
        ...
        public async Task<IActionResult> OnGetExportarPdfAsync(DateTime? fechaDesde, DateTime? fechaHasta, int? cabanaId)
        {
            ...
            datosExportacion.Columnas.AddRange(new List<ColumnaExportacion>
            {
                new() { Nombre = "Medio de Pago", TipoDato = "string", Ancho = 25, Alineacion = "left" },
